                    ...
                    <table class="table table-bordered" id="datatablee">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>
                          <?php if(empty($hdata[17])){
                            if($levellog == "Manager Keuangan"){ ?><a href="#" data-bs-toggle="modal" data-bs-target="#modalProses<?=$hdata['id']?>"><span class="badge bg-warning">On Proses</span></a>
                          <!-- Modal Proses -->
                            <div class="modal fade" id="modalProses<?=$hdata['id']?>" tabindex="-1" aria-labelledby="modalLabel<?=$hdata['id']?>" aria-hidden="true">
                              <div class="modal-dialog">
                                <form method="POST" action="config/proses.php">
                                  <div class="modal-content">
                                    <div class="modal-header">
                                      <h5 class="modal-title" id="modalLabel<?=$hdata['id']?>">Ubah Status Proses Dana</h5>
                                      <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                      <input type="hidden" name="id_pb" value="<?=$hdata['id']?>">
                                    ...
                                    <div class="modal-footer">
                                      <button type="submit" name="simpan_status" class="btn btn-primary">Simpan</button>
                                    </div>
                                  </div>
                                </form>
                              </div>
                            </div>
                            <?php } else { ?> <span class="badge bg-warning">On Proses</span> <?php } } else { ?> <span class="badge bg-success">Sudah Diproses</span> <?php } ?>
                          </td>
                          <td class="text-center">
